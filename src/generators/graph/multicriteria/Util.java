...
public class Util {
	...
	private static void setUpGroup1() {
		...
		q1.addAnswer("On the node, which is connected to the current node with the best edge.", 0,
				"Incorrect! All Edges are treated equally and labels will be created for each outgoing edge");
		q1.addAnswer("On all nodes, that are connected to the current node, including incoming and outcoming edges", 0,
				"Incorrect. Only outgoing edges from the current node are important as the graph is directed");
		...
		q4.addAnswer(
				"The new weights are calculated by adding the according weights of the edge to the current weights of the labels",
				1, "Correct");
		q4.addAnswer("Both weights of the edge will be added to each of the labels weights", 0,
				"Incorrect! The first criterion weight will be added only to the first criterion weight for the label");
		q4.addAnswer("The weights of the edge will be the weights for the new label", 0,
				"Incorrect. Weights are summed up over the duration of the search");
	...
	private static void setUpGroup3() {
		...
		q9.addAnswer("The algorithm stops, as it has found an optimal result", 0,
				"Incorrect! With Pareto optimality multiple results will be optimal and we want to find all of them");
		q9.addAnswer("The algorithm continues to search for additional solutions, as more than one can be optimal", 1,
				"Correct");
		q9.addAnswer("The algorithm continues until it finds the single most optimal solution", 0,
				"Incorrect. With Pareto optimality multiple results will be optimal.");
		...
		q11.setFeedbackForAnswer(false,
				"Incorrect, as the results in the terminal list may be dominated by later labels, therefore, making them not Pareto optimal");
	...
	private static void setUpGroup4() {
		...
		q13.addAnswer("There can be only one label in the labellist for each node: The best one", 0,
				"Incorrect! There can be multiple Labels in the labelllist, as multiple labels can be optimal");
		q13.addAnswer(
				"The label dominated the previous label in the labellist and therefore the dominated label is worse and can be deleted",
				3, "Correct");
		q13.addAnswer("Only the newest labels at the node are saved.", 0,
				"Incorrect. All Pareto optimal Labels at the node are saved");
		...
		q14.setFeedbackForAnswer(false,
				"Incorrect. The labellist for each node saves all Pareto optimal labels at that node");
		...
		q16.setFeedbackForAnswer(false,
				"Incorrect. They may only be used to dominate labels at the specific node they are at");
		...
		/*
		 * MultipleChoiceQuestionModel q17 = new
		 * MultipleChoiceQuestionModel("q17"); q17.setPrompt(labLi5Prompt); q17.
		 * addAnswer("As nodes are visited multiple times, these labels are saved to dominate later ones."
		 * , 3, "Correct");
		 */
	...
	public static void getLabelCreationQuestion(Language language, boolean beforeFor, Label l, Edge e, int numEdges,
			List<String> rightNodes, List<String> allNodes) {
		if (ThreadLocalRandom.current().nextInt(0, 100) > percentageOfQuestions) {
			return;
		}
		...
		if (beforeFor) {
			...
			if (needsAnswer.contains(q.getID())) {
				ArrayList<Answer> answers = getAnswerForLabelCreationQuestion(q.getID(), numEdges, rightNodes,
						allNodes);
	...
	private static ArrayList<Answer> getAnswerForLabelCreationQuestion(String id, int numberOfEdges,
			List<String> rightNodes, List<String> allNodes) {
		ArrayList<Answer> answers = new ArrayList<Answer>();
		if (id.equals("q2")) {
			// NODELIST
			answers.add(new Answer(rightNodes.stream().reduce("", (a, b) -> {
				return a + "," + b;
			}).substring(1), "Correct!", 1, true));
			for (int i = 0; i < 3; i++) {
				List<String> wrongNodes = getWrongNodeList(allNodes, rightNodes);
				answers.add(new Answer(wrongNodes.stream().reduce("", (a, b) -> {
					return a + "," + b;
				}).substring(1), "Incorrect!!", 0, false));
		...
		if (id.equals("q5")) {
			...
			answers.add(new Answer(String.valueOf(numberOfEdges + ThreadLocalRandom.current().nextInt(4, 7)),
					"Incorrect!", 0, false));
			answers.add(new Answer(String.valueOf(numberOfEdges - ThreadLocalRandom.current().nextInt(4, 7)),
					"Incorrect!", 0, false));
			answers.add(new Answer(String.valueOf(numberOfEdges + ThreadLocalRandom.current().nextInt(1, 4)),
					"Incorrect!", 0, false));
			answers.add(new Answer(String.valueOf(numberOfEdges - ThreadLocalRandom.current().nextInt(1, 4)),
					"Incorrect!", 0, false));
		}
	...
	private static List<String> getWrongNodeList(List<String> allNodes, List<String> rightNodes) {
		...
		while (true) {
			wrongNodes = allNodes.stream().filter(x -> ThreadLocalRandom.current().nextBoolean())
	...
	public static void getDominatedQuestion(Language language, int state, Label l, Label newL, String lstr,
			String newLstr) {
		if (ThreadLocalRandom.current().nextInt(0, 100) > percentageOfQuestions) {
			return;
		}
	...
	private static ArrayList<Answer> getDominatedAnswers(boolean b, Label l, Label newL) {
		...
		if (b) {
			...
			answers.add(new Answer(wrong4,
					"Incorrect. Domination is always required to only keep the Pareto optimal labels at a node.", 0,
					false));
